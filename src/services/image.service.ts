...
export class ImageService {
     ...
     */
    async uploadProductImage(tenantId: string, productId: string, file: Express.Multer.File) {
        try {
            if (!file) {
                return {
                    success: false,
     ...
     */
    async updateImageMetadata(imageId: string, altText: string, caption?: string) {
        try {
            const updated = await prisma.productImage.update({
                where: { id: imageId },
                data: {
     ...
     */
    async updateStoreBranding(tenantId: string, branding: any) {
        try {
            const {
                primary_color,
                secondary_color,
     ...
     * Get store branding
     */
    async getStoreBranding(tenantId: string) {
        try {
     ...
     */
    async generateBrandingCSS(tenantId: string) {
        try {
...
.accent-text {
  color: var(--accent-color);
     ...
     */
    async bulkUploadImages(tenantId: string, productId: string, files: Express.Multer.File[]) {
        try {
            const results: any[] = [];
