  ...
  "titles": ["title1", "title2", ...]
    ...
    async autoGenerateProductSEO(
        ...
        try {
            ...
            const descResult = await this.generateProductDescription(
                productName,
                category,
                nicheType,
                imageUrl
            ...
            const keywordResult = await this.generateSEOKeywords(
                productName,
                description,
                category,
                nicheType
            ...
            const titleResult = await this.optimizeProductTitle(
                productName,
                category,
                nicheType
            );
            ...
            const structuredResult = await this.generateStructuredData(
                productName,
                description,
                price || 0,
                category,
     ...
     */
    async updateProductSEO(productId: string, updates: any) {
        try {
     ...
     */
    async calculateSEOScore(productId: string) {
        try {
            const seoData = await prisma.productSEO.findUnique({
                where: { productId: productId }
            });
     ...
     */
    async batchGenerateSEO(products: any[]) {
        try {
            const results: any[] = [];

            for (const product of products) {
                const result = await this.autoGenerateProductSEO(
                    product.id,
                    product.name,
                    product.category,
                    product.nicheType,
