 ...
 * Save business info
 */
router.post('/step-1', asyncHandler(async (req, res) => {
 ...
 * Save branding info
 */
router.post('/step-2', asyncHandler(async (req, res) => {
    const tenantId = req.user.tenantId;
 ...
 */
router.post('/step-3', asyncHandler(async (req, res) => {
    const tenantId = req.user.tenantId;
...
router.post('/launch', asyncHandler(async (req, res) => {
    ...
    const template = nicheTemplate;
    if (template.defaultProducts) {
        await prisma.product.createMany({
            data: template.defaultProducts.map((p) => ({
                tenant_id: tenantId,
                name: p.name,
                price: p.price,
