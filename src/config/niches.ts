...
export interface NicheFeatures {
    ...
    color: string;
    icon: string;
    features: { lite: string[]; rise: string[]; elite: string[] };
    workflows: { lite: string[]; rise: string[]; elite: string[] };
    integrations: { lite: string[]; rise: string[]; elite: string[] };
}

// Pharmacy - 💊
export const PHARMACY: NicheFeatures = {
    id: 'pharmacy',
    name: 'Pharmacy',
...
// Restaurant - 🍽️
export const RESTAURANT: NicheFeatures = {
    id: 'restaurant',
    name: 'Restaurant',
...
// Real Estate - 🏠
export const REAL_ESTATE: NicheFeatures = {
    id: 'real_estate',
    name: 'Real Estate',
...
// Church - ⛪
export const CHURCH: NicheFeatures = {
    id: 'church',
    name: 'Church',
...
// Electronics - 📱
export const ELECTRONICS: NicheFeatures = {
    id: 'electronics',
...
// Salon - 💇
export const SALON: NicheFeatures = {
    id: 'salon',
    name: 'Salon / SPA',
...
// Boutique - 👗
export const BOUTIQUE: NicheFeatures = {
    id: 'boutique',
...
// Hardware - 🔨
export const HARDWARE: NicheFeatures = {
    id: 'hardware',
    name: 'Hardware Store',
...
// General Store B2B - 🏪
export const GENERAL_STORE: NicheFeatures = {
    id: 'general_store',
...
// Clinic - 🏥
export const CLINIC: NicheFeatures = {
    id: 'clinic',
    name: 'Clinic / Medical',
...
// Registry
const NICHES_REGISTRY: Record<Niche, NicheFeatures> = {
    pharmacy: PHARMACY,
...
// Export functions
export function getNiche(id: Niche): NicheFeatures | null {
    return NICHES_REGISTRY[id] || null;
}

export function listNiches(): NicheFeatures[] {
    return Object.values(NICHES_REGISTRY);
}

export function nicheExists(id: string): id is Niche {
    return id in NICHES_REGISTRY;
}

export function getNicheFeatures(id: Niche, tier: 'lite' | 'rise' | 'elite'): string[] {
    const niche = getNiche(id);
    return niche ? (niche.features[tier] || []) : [];
}

export function hasFeature(id: Niche, tier: 'lite' | 'rise' | 'elite', feature: string): boolean {
    return getNicheFeatures(id, tier).includes(feature);
}

export function getAllFeaturesUpToTier(id: Niche, tier: 'lite' | 'rise' | 'elite'): string[] {
    const niche = getNiche(id);
    if (!niche) return [];
    const features = [...(niche.features.lite || [])];
    if (tier === 'rise' || tier === 'elite') features.push(...(niche.features.rise || []));
    if (tier === 'elite') features.push(...(niche.features.elite || []));
    return features;
}
